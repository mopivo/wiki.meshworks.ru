...
2. CI автоматически собирает артефакт (`wiki-build`). Раскатывание — Actions → **Deploy wiki** → **Run workflow** (`main`).
3. Дождитесь завершения workflow: он скачает артефакт, зальёт готовый `build/` на сервер и просто перезапустит сервис (без повторного `npm run build`). Логи доступны в GitHub Actions.
